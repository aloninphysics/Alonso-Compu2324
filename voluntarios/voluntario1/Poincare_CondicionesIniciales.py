...
for (x, y), color, etiqueta in zip(datos, colores, etiquetas):
    plt.plot(x, y, color=color, alpha=0.5)  # Remover el parámetro label

plt.xlabel('$\phi$ (rad)')
plt.ylabel('$\dot{\phi}$ (rad/s)')
plt.title('Mapa de Poincaré para $\phi$ y $\dot{\phi}$ para condiciones iniciales $\phi=$ $\psi=1.0$ rad, $\dot{\phi}=$ $\dot{\psi}=1.0$ rad/s ')
plt.grid(True)
